...
Additonally, multiple demonstrations are live online:
...
- **Infinite Memory** - An endless amount of data can be stored using only a few bytes
- **Re-Creation** - The same data is generated every time, as long as the seed and parameters stay the same
- **Security** - Data obfuscation can be obtained by using custom parameters in the main engine
- **Reversible** - The data chain can be traversed forward and backward for its entire length
- **Indexible** - Any state in the output list can be jumped to from anywhere else in the data array
- **Versatility** - Can be used in applications featuring content generation, synthetic data and procedural generation
...
From [`Github`](https://github.com/jgddesigns/stateshaper)
```bash
git clone https://github.com/jgddesigns/stateshaper.git
```
<br>

**Make sure your data is in one of the formats listed in the *"example_data"* directory of the repository. The output that is generated depends on the values contained in this dataset. The data types are based on the following rules:**

*If needed, use the *FormatData* class in the nested *'format_data'* directory.*

Instructions: [`Formatting Data for Input`](https://github.com/jgddesigns/stateshaper/blob/main/example_data/format)<br> 
...
Example: [`Random Dataset`](https://github.com/jgddesigns/stateshaper/blob/main/example_data/random.json)
...
Example: [`Compound Dataset`](https://github.com/jgddesigns/stateshaper/blob/main/example_data/compound.json)

<br> 

*Rating* - Creates a sense of personalization for the output. The output is initially created based on a ratings preference. Afterward, the output is derived from the current included items and adjusted based on whatever parameters are decided upon (such as user input). The *'derived'* dataset is all that needs to be saved on the backend, and does not include a *'rating'* key. It can be used for all profiles in an application. Either set can be used after the initial run. If the initial set is used, the ratings won't be included in the derivations after the first run that uses that data.

Example: [`Initial Rating Dataset`](https://github.com/jgddesigns/stateshaper/blob/main/example_data/rating_initial.json), [`Derived Rating Dataset`](https://github.com/jgddesigns/stateshaper/blob/main/example_data/rating_derived.json) 
...
*Tokens* - Generates a deterministic set of numbers that can be tokenized for use in algorithms. Needs additional data or input. Modifying the core input parameter values will create different numerical output.

Example: [`Tokens Dataset`](https://github.com/jgddesigns/stateshaper/blob/main/example_data/tokens.json)

<br> <br> 

**Initialize a *Stateshaper* class:**
...
# constants (optional, default={"a": 3, "b": 5, "c": 7, "d": 11}) - Only change this for custom morphing equations or security purposes.
...
#
# custom initial_state, constants and mod values can be used to enhance privacy and/or create variation in the output

from stateshaper import *

# BASIC (first run)
engine = Stateshaper(data=your_data, token_count=needed_tokens)

# RE-CREATE PREVIOUS OUTPUT
engine = Stateshaper(data=your_data, seed=created_seed, token_count=needed_tokens)

# CUSTOM
engine = Stateshaper(data=your_data, seed=created_seed, token_count=needed_tokens, constants=optional_custom_logic, mod=more_optional_logic)

#FULLY DEFINED 
engine = Stateshaper(data=your_data, token_count=needed_tokens, initial_state=optional_int_or_int_list, constants=optional_custom_logic, mod=more_optional_logic)
...
**Run the engine to generate output:**
...
For continuous use, the engine can be called in a loop using these functions. For one time, call it once with a specific *token_count* parameter.
...
For more info, see the [`CONNECTOR`](https://github.com/jgddesigns/stateshaper/blob/main/src/main/connector/CONNECTOR.md) documentation.  
...
Aside from the plugin file (which can be a template that does not include specific numbers), relevant data such as the input dataset and ratings can be condensed into *Tiny State* and/or *Raw State* format. Example:
...
For more info, see the [`TINY_STATE`](https://github.com/jgddesigns/stateshaper/blob/main/src/main/tools/tiny_state/TINY_STATE.md) documentation.
...
***1. Define the Data***
What will be the final output? This can be an array of any type of values, including functions.

***2. Are Custom 'seed', 'constants' or 'mod' Values Needed?***
If specific deterministic output is needed, these values can be adjusted to fit with the morph equation.
...
For more info, see the [`TESTS`](https://github.com/jgddesigns/stateshaper/blob/main/src/main/tests/TESTS.md) documentation.
...
See [`CONTRIBUTING`](https://github.com/jgddesigns/stateshaper/blob/main/CONTRIBUTING.md) instructions if you are interested in creating a custom plugin (or anything else). Right now you can fork this repo to experiment. An open source version of the code will be available soon. 
...
This project is released under the MIT License. See [`LICENSE`](https://github.com/jgddesigns/stateshaper/blob/main/LICENSE) for details.
